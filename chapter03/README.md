...
<table>
  ...
  <tbody>
   ...
   project manager found a code that can be entered into the game to increase
   the AI’s chances of beating the game. You are given the Konami *code* and a
   virtual *controller* to enter the code into.

   `code` is an object that contains a sequence of keypresses, stored as
   strings. `code` remembers where it is in the sequence, so the keypresses are
   accessed through the functions `next()`, `prev()`, and `curr()`.

   *   `code.next()` changes the current keypress of the sequence to the next
       one. It returns the next keypress if there is one, or `undefined` if
       there are no more.
   *   `code.prev()` changes the current keypress of the sequence to the
       previous one. It returns the previous keypress if there is one, or
       `undefined` if there are none to go back to.
   *   `code.curr()` returns the current keypress in the sequence.

   `controller` is a function that accepts a string. When it recives the correct
   string for a sequence and is ready to receive another string in the sequence,
